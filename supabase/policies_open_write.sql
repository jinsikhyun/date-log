...
-- 공개 수정·삭제 정책을 설치했다. 실사용자 커플이 생긴 지금 이 SQL 은 단 한 줄도
...
--     git log --follow -p -- supabase/policies_open_write.sql
--
-- 현재 정책의 근거 파일:
...
do $$ begin
  raise exception '폐기된 스크립트입니다 — supabase/policies_open_write.sql 는 실행할 수 없습니다. 커플 격리를 되돌리는 내용이었고, 현재 정책은 add-couple-rls.sql 과 security/01·02 가 담당합니다. (legacy script removed)';
end $$;

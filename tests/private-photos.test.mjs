...
function load(file, imports = {}) {
 ...
 vm.runInNewContext(output,{exports,require:name=>{
  ...
  return imports[name];
 },process:{env:{NEXT_PUBLIC_SUPABASE_URL:'https://example.supabase.co'}},URL,Response});
 return exports;
}
...
function handler({user=true,allowed=true,accessError=null,downloadError=null,type='image/jpeg'}={}){
 ...
 storage:{from:bucket=>({download:async path=>{calls.push(['download',bucket,path]);return {data:downloadError?null:new Blob(['fake-image'],{type}),error:downloadError};}})}};
 const route=load('src/app/api/place-photo/route.ts',{'@/lib/supabase/server':{createClient:async()=>client},'@/lib/photoUrls':urls});
 return {get:path=>route.GET(new Request('https://app.invalid/api/place-photo?path='+encodeURIComponent(path))),calls};
}

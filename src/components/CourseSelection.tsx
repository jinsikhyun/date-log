...
export function useCourseSelection() {
  ...
  const trigger = <button type="button" className="shrink-0 whitespace-nowrap rounded-full px-2.5 py-2 text-[11px] font-medium text-muted transition-colors hover:bg-accent/5 hover:text-accent sm:px-3 sm:text-xs" onClick={() => { setActive(!active); setIds([]); }}>{active ? "선택 취소" : <>코스<span className="hidden sm:inline"> 조합하기</span></>}</button>;
